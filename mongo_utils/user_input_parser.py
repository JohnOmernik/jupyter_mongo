...
class UserInputParser(ArgumentParser):
    ...
    def __init__(self, *args, **kwargs):
        self.valid_commands = list(filter(lambda func: not func.startswith("_") and hasattr(getattr(MongoAPI, func), "__call__"), dir(MongoAPI)))
        ...
        self.parser_show_databases = self.line_subparsers.add_parser("show_dbs", help="Show database names in your current connection")
        self.parser_show_databases.add_argument("-i", "--instance", required=True, help="the instance to run the command against")
        
        # Subparser for "show_collections"
        self.parser_show_collections = self.line_subparsers.add_parser("show_collections", help="Show the collections in a database")
        self.parser_show_collections.add_argument("-i", "--instance", required=True, help="the instance to run the command against")
        self.parser_show_collections.add_argument("-d", "--database", required=True, help="the name of the database to examine")
        
        ## CELL SUBPARSERS ##
        # Subparser for "find_one"
        self.parser_find_one = self.cell_subparsers.add_parser("find_one", help="Query the collection for a single document matching a query")
        self.parser_find_one.add_argument("-i", "--instance", required=True, help="the instance to run the command against")
        self.parser_find_one.add_argument("-d", "--database", required=True, help="the name of the database that contains the collection")
        self.parser_find_one.add_argument("-c", "--collection", required=True, help="the name of the collection to query")
        ...
        self.parser_find.add_argument("-d", "--database", required=True, help="the name of the database that contains the collection")
        ...
        self.parser_count_documents = self.cell_subparsers.add_parser("count_documents", help="Count the number of documents in a collection")
        self.parser_count_documents.add_argument("-i", "--instance", required=True, help="the instance to run the command against")
    ...
    def transform_query(self, query):
        """Split a user-supplied Mongo query string into a list
    ...
    def parse_input(self, input, type):
        ...
        if type == "line":        
            try:
                ...
                parsed_input["error"] = True
                parsed_input["message"] = r"Invalid input received, see the output above. Try `%mongo --help` or `%mongo -h`"
        ...
        if type == "cell":
            ...
            try:
                if len(split_user_input) == 1:
                    ...
                    parsed_input["input"].update(vars(parsed_user_command))
                    parsed_input["error"] = True
                    parsed_input["message"] = "Expected to get 2 lines in your cell magic, but got 1. Did you forget to include a query?\nTry `--help` or `-h`"
